...
SCAN_NOW_RATE_FILE = Path("/tmp/optionspro_scan_now_last_request")
SCAN_NOW_RATE_SECONDS = 300  # one manual scan per 5 minutes
...
class H(http.server.BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if p=="/api/signals":
            ...
            try:
                ...
                payload["market_status"] = live_status
                # Manual-scan status so the dashboard can show "Scan queued..."
                payload["scan_pending"] = SCAN_NOW_FILE.exists()
                # Compute when the user can request another manual scan
                try:
                    last_scan_req = float(SCAN_NOW_RATE_FILE.read_text().strip()) if SCAN_NOW_RATE_FILE.exists() else 0.0
                    cooldown_left = max(0, int(SCAN_NOW_RATE_SECONDS - (time.time() - last_scan_req)))
                    payload["scan_cooldown_secs"] = cooldown_left
                except Exception:
                    payload["scan_cooldown_secs"] = 0
                if not SIGNALS_FILE.exists():
                    payload["message"] = (
    ...
    def do_POST(self):
        ...
        if p=="/api/trigger-scan":
            # Drop a marker file the engine picks up at the top of its monitor loop.
            # Rate-limited: one manual scan per 5 minutes. The engine will only
            # honor the marker if the market is open and TWS is connected;
            # otherwise it deletes the marker and logs "ignored".
            try:
                now = time.time()
                last = 0.0
                if SCAN_NOW_RATE_FILE.exists():
                    try: last = float(SCAN_NOW_RATE_FILE.read_text().strip() or "0")
                    except: last = 0.0
                remaining = int(SCAN_NOW_RATE_SECONDS - (now - last))
                if remaining > 0:
                    self._json({
                        "ok": False,
                        "error": f"Manual scan can only be triggered once every {SCAN_NOW_RATE_SECONDS // 60} minutes.",
                        "retry_after_secs": remaining,
                    }, 429);return
                # Write marker; engine reads on next loop iteration (within ~30s typically)
                SCAN_NOW_FILE.write_text(now_iso() + "\n")
                SCAN_NOW_RATE_FILE.write_text(str(now))
                # Try chmod 600 — best-effort
                try: SCAN_NOW_FILE.chmod(0o600)
                except: pass
                self._json({
                    "ok": True,
                    "queued_at": now_iso(),
                    "marker_file": str(SCAN_NOW_FILE),
                    "expected_within_secs": 60,
                    "next_request_in_secs": SCAN_NOW_RATE_SECONDS,
                });return
            except Exception as e: self._json({"ok":False,"error":str(e)},500);return
        if p=="/api/real-rules":
            try:
